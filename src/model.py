...
class Model(nn.Module):
    def __init__(
        ...
        self.transformers = nn.ModuleList(
            [
                TransformerLayer(heads_count, ff_network_size, context_size, embedding_size, self.dropout_rate)
                for _ in range(transformers_count)
            ]
        )
...
class SelfAttentionLayer(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor) -> Tensor:
        ...
        # [batch; heads_count; sequence_size; embeddings_size_per_head]
        ...
        attention_weights = q @ k.transpose(2, 3)
        ...
        # [batch; heads_count; sequence_size; sequence_size]
        attention_weights = torch.softmax(attention_weights * self.attention_scale, dim=-1)
        ...
        # [batch; sequence_size; heads_count; embedding_size]
        attention_value = attention_value.transpose(1, 2)

        # Reshape attention_value to [batch; sequence_size; embedding_size]
        # [batch; sequence_size; embedding_size]
        attention_value = attention_value.reshape(B, S, self.embedding_size)
